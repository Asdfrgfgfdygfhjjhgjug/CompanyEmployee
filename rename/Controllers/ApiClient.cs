namespace CompanyEmployee.Web.Controllers
{
    ...
    public class ApiClient : IApiClient
    {
        ...
        public ApiClient(HttpClient client)    //, Uri baseEndpoint 
        {
            httpClient = client;
        ...
        {
            addHeaders();
            var test = CreateHttpContent<T>(content);
            var response = await httpClient.PostAsync(requestUrl.ToString(), new StringContent("{ \"firstName\": \"John\" }", System.Text.Encoding.UTF8, "application/json"));
            response.EnsureSuccessStatusCode();
            var data = await response.Content.ReadAsStringAsync();
